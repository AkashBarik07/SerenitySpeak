    ...
    <div>
      ...
      <div className="flex justify-center mb-7 flex-wrap gap-y-5 gap-x-8 mt-6">
        {mediation.map((data, index) => (
          <div
            key={index}
            className="card card-compact bg-neutral-900 md:w-80  shadow-xl rounded-2xl overflow-hidden "
          ...
          >
            ...
            <div className="card-body items-center text-center pb-4">
              <h2 className="card-title text-teal-400 justify-center items-center p-2 ">{data.title}</h2>
              <p className="text-center font-medium text-white pb-5">{data.author}</p>
              <div className="card-actions justify-center">
                <a href={data.video} target="_blank" rel="noopener noreferrer">
                  <button className="bg-sky-600 hover:bg-red-600 py-2 text-black font-semibold px-3 rounded-lg">
                    Watch Now
                  </button>
                </a>
              </div>
